...
from sqlmodel_crud.exception import MultipleResultsFound, NotFound
...
class TestAddToSession:
    ...
    def test_update(self, service: PlayerService, query_service: PlayerService) -> None:
        ...
        players2 = service.exec(service.select().order_by(DbPlayer.name)).all()
        for first, second in zip(players, players2):
